...
def main():
    try:
        ...
        commands = []
        commands.append(("Run all doctests", lab_tests + readme_tests))
        commands.append(("Run all doctests in lab.py", lab_tests))
        commands.append(("Run all doctests in readme.md", readme_tests))
        ...
        while not (isinstance(option, int) and 0 <= option < len(commands)):
            option = input("Pick an option [0-{}]: ".format(len(commands) - 1))
